...
import { createProduct, toggleProductActive, updateProduct, deleteProduct, publishToWebCatalog } from "./actions";
...
// ============================================================
// PUBLISH TO WEB BUTTON + MODAL
// ============================================================

function PublishWebButton({ product }: { product: ProductWithCategory }) {
  const { toast } = useToast();
  const [isOpen, setIsOpen] = useState(false);
  const [isPending, startTransition] = useTransition();
    ...
    startTransition(async () => {
      ...
      if ("error" in result) {
        ...
        toast({
          ...
          description: `SKU ${product.sku} está no catálogo da vitrine.`,
        });
        handleClose();
      }
    });
  ...
  const catCode = product.categories?.code ?? "OUTR";
                    ...
                    >
                      ...
                      <td className="px-5 py-3">
                        <div className="flex items-center gap-2">
                          ...
                          />
                          <PublishWebButton product={product} />
                          <DeleteProductButton
                            sku={product.sku}
                            name={product.name}
                          />
